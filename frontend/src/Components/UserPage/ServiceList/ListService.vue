...
<template>
  <div>
    <div class="flex items-center justify-center p-2 flex-wrap bg-pink-500 sticky top-0 z-50 md:flex-nowrap">
      <button type="button" @click="fetchService"
      ...
      <button v-for="category in useCategory.categories" :key="category.id" type="button"
        @click="getByCategoryId(category.id)"
      ...
      <div class="relative w-full md:w-auto ml-5 mt-3 md:mt-0">
        ...
        <input type="text" id="search-navbar" v-model="searchQuery" @input="searchService"
          class="block mb-2 w-full p-2 pl-10 text-sm text-blue-900 border border-gray-300 rounded-full bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
          placeholder="Search..." />
      </div>
    </div>
        ...
        <div class="sticky top-40">
          <div class="bg-white rounded-lg shadow-md overflow-hidden p-3">
            ...
            <div class="border-t mt-4 pt-4">
              ...
              <button v-if="cardItems.cards.length === 0"
                class="mt-4 bg-pink-500 text-white w-full py-2 rounded-lg shadow-md" disabled>
